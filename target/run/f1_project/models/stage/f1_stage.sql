...
SELECT distinct c.name, 
  ...
  r.year
FROM `zoomcampproject`.`f1_data_all`.`external_table_constructor_results` cr
inner join `zoomcampproject`.`f1_data_all`.`external_table_races` r
  on r.raceid = cr.raceid
inner join `zoomcampproject`.`f1_data_all`.`external_table_constructors` c
